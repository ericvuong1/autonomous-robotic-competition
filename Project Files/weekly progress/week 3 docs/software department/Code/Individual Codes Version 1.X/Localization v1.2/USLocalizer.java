...
public class USLocalizer {
	...
	private double[] getLocation() {
		...
		dist[1] = USAdjustment(getFilteredData())-30;
		odo.getNavigator().sleep(200);
		odo.getNavigator().turn(-90);
		dist[0] = USAdjustment(getFilteredData())-30;
		odo.getNavigator().turn(180);
		return dist;
	}

	private double USAdjustment(double input) {
		double adjustment = 0.00000173 * Math.pow(input, 5) - 0.000172
				* Math.pow(input, 4) + 0.00867 * Math.pow(input, 3) - 0.1685
				* Math.pow(input, 2) + 2.22*input;
		return adjustment;
	}
