...
public class USLocalizer {
	...
	private final int filterValue = 55; // to filter out large value for US
	...
	private final double angleAdjustment = 0; //based on test results for advanced localization
	...
	private final double xAdjustment = 4.829;
	private final double yAdjustment = 0.791;
	...
	public void doLocalization() {
		...
		doAdvancedLocalization();
		
		//doOrientation(); doCoordinate();

	}

	private void doAdvancedLocalization() {
		...
		double[] angle = new double[2]; // storing the angle when min is detected
		...
		if (currentDist - nextDist > 0) { // to detect if there is a negative slope in
		...
		if (angleDifference < 180) {
			odo.setX(minimum[0] - 30 + centertoUS);
			odo.setY(minimum[1] - 30 + centertoUS);
			double adjustedAngle = odo.getAngle() + 270 - angle[1] + angleAdjustment;
			odo.setTheta(adjustedAngle*Math.PI/180);
		}
		if (angleDifference > 180) {
			odo.setX(minimum[1] - 30 + centertoUS);
			odo.setY(minimum[0] - 30 + centertoUS);
			double adjustedAngle = odo.getAngle() + 180 - angle[1] + angleAdjustment;
			odo.setTheta(adjustedAngle*Math.PI/180);
		}
	}
