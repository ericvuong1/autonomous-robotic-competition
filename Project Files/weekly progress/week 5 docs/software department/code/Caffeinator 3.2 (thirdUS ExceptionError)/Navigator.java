...
public class Navigator extends Thread {
	...
	private UltrasonicSensor frontUS, sideUS, oSideUS;
	...
	public static int sideData, frontData, oSideData;

	private final Odometer odometer;

	public Navigator(Odometer odometer, UltrasonicSensor frontUS,
			UltrasonicSensor sideUS, UltrasonicSensor oSideUS) {
	...
	private static final double NAVIGATOR_TOLERANCE_DIST = 0.8;
...
//			}
			...
			if (sideDist() < 10 && avoidance== true) {
				driveDist(-2);
				turnTo(wrapAngle(currentTheta - Math.PI / 4));
				doAvoidanceLeft(x, y);
			} 
			if(oSideDist()<10 && avoidance == true)
				{
					driveDist(-2);
					turnTo(wrapAngle(currentTheta+Math.PI/4));
					doAvoidanceRight(x,y);
				}
			else {
				leftMotor.setSpeed(motorStraight);
				rightMotor.setSpeed(motorStraight);
			}
			if (frontDist() < dangerZone && avoid == false && avoidance == true) {
				driveDist(-6);
	...
	public void doAvoidanceRight(double x, double y) {
		avoid = true;
		...
		while (avoid) {
			...
			if (oSideDist() > wallFollowerFar) {
				leftMotor.setSpeed(motorHigh);
				rightMotor.setSpeed(motorLowLow + 20);
			} else if (oSideDist() < wallFollowerClose) {
	...
	public void doAvoidanceLeft(double x, double y) {
		avoid = true;
		// main loop for avoiding an obstacle
		while (avoid) {
			double currentTheta = odometer.getTheta();
