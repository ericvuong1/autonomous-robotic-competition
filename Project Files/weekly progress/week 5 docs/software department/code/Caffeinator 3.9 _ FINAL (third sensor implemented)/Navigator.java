...
public class Navigator extends Thread {
	...
	private double initiateObsDist = 20;// threshold before avoiding an obstacle
	...
	public void travelTo(double x, double y, boolean avoidance) {
		...
		while (navigating) {
			...
			if(odometer.getX() > 6*30.48)
			{
				condition = false;
			}
			if (leftDist() < initiateObsDist && avoidance == true && condition == true) {
				//driveDist(-2);
			...
			if (rightDist() < initiateObsDist && avoidance == true && condition == true) {
				//driveDist(-2);
				turnTo(wrapAngle(currentTheta + Math.PI / 4));
				NAVIGATOR_TOLERANCE_DIST = 4;
