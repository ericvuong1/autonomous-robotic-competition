...
public class Navigator extends Thread {
	...
	private double initiateObsDist = 25;// threshold before avoiding an obstacle
	...
	public void travelTo(double x, double y, boolean avoidance) {
		...
		while (navigating) {
			...
			if (frontDist() < initiateObsDist && avoid == false && avoidance == true) {
				driveDist(-6);
	...
	public void doPAvoidance(double x, double y) {
		avoid = true;

		while (avoid) {
			double currentTheta = odometer.getTheta();
			int distance = sideDist();
			...
			if (distance > this.bandCenter) { // robot is too far away from the
				...
				leftMotor.setSpeed(this.motorStraight
						- (distance - this.bandCenter) * 5);
				leftMotor.forward();
				// equation for left motor

				rightMotor
						.setSpeed((5 * (distance - this.bandCenter) + this.motorStraight));
				rightMotor.forward();
